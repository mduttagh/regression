...
fit1 <-
  lm(SALE.PRICE ~ NET.VALUE + ACREAGE + SALE.ASSESSMENT + LAND.VALUE + IMPROVEMENT.VALUE + NEW.HOME,data = df1)
...
fit2 <-
  lm(SALE.PRICE ~ SALE.ASSESSMENT + LAND.VALUE + NEW.HOME, data = df2)
...
out2 <- mlreg(df = df2[vars2],
              fit = fit2,
              2)
df2$NEW.HOME.org <- df2$NEW.HOME
df2$NEW.HOME <- df2$NEW.HOME + 1
powerT(df = df2[vars2],
       fit = fit2,
       2)
df2$NEW.HOME <- df2$NEW.HOME.org
...
fit3 <-
  lm(
    SALE.PRICE ~ SALE.ASSESSMENT + NEW.HOME,
    data = df3
  )
...
out3 <- mlreg(df = df3[vars3],
              fit = fit3,
              3)
df3$NEW.HOME.org <- df3$NEW.HOME
df3$NEW.HOME <- df3$NEW.HOME + 1
powerT(df = df3[vars3],
       fit = fit3,
       3)
df3$NEW.HOME <- df3$NEW.HOME.org
