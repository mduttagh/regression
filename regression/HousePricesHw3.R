...
df1 <-
  df0 %>% filter(
    ...
    SALE.PRICE >= NET.VALUE,
    YEAR.CONSTRUCTED >= "2000-01-01"
...
# df1$BEDS[df1$BEDS==0] <- NA
# df1$BATHS[df1$BATHS==0] <- NA
# df1$ACREAGE[df1$ACREAGE==0] <- NA
df1$COUNTY.DISTRICT <- factor(df1$COUNTY.DISTRICT)
df1$BLOCK <- factor(df1$BLOCK)
df1$LOT <- factor(df1$LOT)
df1$RECORD.ID <- factor(df1$RECORD.ID)
df1$TRANSACTION.UPDATE.NO <- factor(df1$TRANSACTION.UPDATE.NO)
df1$BUILDING.DESCRIPTION <- factor(df1$BUILDING.DESCRIPTION)
df1$ACREAGE <- as.numeric(df1$ACREAGE)
df1$TRANSACTION.DATE.MMDDYY <-
  as.Date(df1$TRANSACTION.DATE.MMDDYY, "%m%d%y")
df1$DEED.DATE.MMDDYY <- as.Date(df1$DEED.DATE.MMDDYY, "%m%d%y")
df1$PROPERTY.CLASS <- factor(df1$PROPERTY.CLASS)
df1$BUILDING.CLASS.CODE <- factor(df1$BUILDING.CLASS.CODE)
df1$BILL.STATUS.FLAG <- factor(df1$BILL.STATUS.FLAG)
df1$NO.OF.DWELLINGS <- factor(df1$NO.OF.DWELLINGS)
df1$YEAR.CONSTRUCTED <- as.Date(df1$YEAR.CONSTRUCTED, "%Y")
df1$SALE.PRICE <- as.numeric(df1$SALE.PRICE)
df1$SALE.ASSESSMENT <- as.numeric(df1$SALE.ASSESSMENT)
df1$SALES.PRICE.CODE <- factor(df1$SALES.PRICE.CODE)
df1$LAND.VALUE <- as.numeric(df1$LAND.VALUE)
df1$IMPROVEMENT.VALUE <- as.numeric(df1$IMPROVEMENT.VALUE)
df1$NET.VALUE <- as.numeric(df1$NET.VALUE)
df1$LAST.YEAR.TAX <- NULL
df1$PRIOR.YR.NET.VALUE <- NULL
df1$ZIP.CODE <- factor(df1$ZIP.CODE)
glimpse(df1)
summary(df1)
df1 <-
  as.data.frame(
    df1    %>% filter(FLOOR.AREA > 100) %>% dplyr::select(
      -contains("FILLER"),
      -starts_with("ADDITION"),
      -starts_with("DEDUCTION"),
      -contains("SR1A"),
      -starts_with("EPL"),
      -starts_with("STATUTE"),
      -starts_with("LAND.VALUE"),
      -starts_with("IMPROVEMENT"),
      -starts_with("SPECIAL"),
      -starts_with("EXEMPTION"),
      -ends_with("CNT"),
      -contains("REBATE"),
      -contains("MUNICIPAL"),
      -starts_with("BILL.STATUS.FLAG"),
      -starts_with(("STATEMENT"))
    )
  )
#rownames(df1) <- paste(df1$BLOCK,df1$LOT,df1$QUALIFIER,df1$RECORD.ID)
#df1 <- sample_n(df1,500)
#df1$OLD <- ifelse(df1$YEAR.CONSTRUCTED <= "2005-01-01", TRUE, FALSE)
# Select more buildings
df11 <-
  df0 %>% filter(
...
addr11 <- df11 %>% dplyr::select(STREET.ADDRESS,CITY.STATE,ZIP.CODE,BEDS,BATHS,FLOOR.AREA)
write.table(addr11,file="realtorSqFt2.csv",row.names = FALSE, sep = "|")
fit1 <-
  lm(SALE.PRICE ~ NET.VALUE + FLOOR.AREA + ACREAGE + SALE.ASSESSMENT + OLD + BEDS + BATHS,
     data = df1)
...
df2 <-
  df2 %>% filter(ACREAGE > 0) %>% mutate(
    SALE.PRICE.SQR = SALE.PRICE^2,
    NET.VALUE.SQR = NET.VALUE^2,
    FLOOR.AREA.SQR = FLOOR.AREA^2,
    ACREAGE.SQR = ACREAGE^2
  ) %>% na.omit() 
df2 <- subset(df2, !(row.names(df2) %in% out2)) # For iterations
fit2 <-
  lm(SALE.PRICE.SQR ~ NET.VALUE.SQR + FLOOR.AREA.SQR + ACREAGE.SQR  + OLD,
     data = df2)
...
df3 <-
  df3 %>% mutate(
    ...
    NET.VALUE.SQRT = sqrt(NET.VALUE),
    FLOOR.AREA.SQRT = sqrt(FLOOR.AREA),
    ACREAGE.SQRT = sqrt(ACREAGE)
  )# %>% na.omit()
out3 <- ret
...
fit3 <-
  lm(SALE.PRICE ~ NET.VALUE.SQRT + FLOOR.AREA.SQRT + ACREAGE.SQRT  + OLD + BATHS, data = df3)
vars <- colnames(model.frame(fit3, df3))
df3 <-
  as.data.frame(df3 %>% dplyr::select(one_of(vars), everything()))
out3 <- NULL
out3 <- mlreg(df = df3,
              fit = fit3,
              3)
