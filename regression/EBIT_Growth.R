...
# Data http://pages.stern.nyu.edu/~adamodar/New_Home_Page/datafile/fundgrEB.html

cat("\n\nFirst Round\n")
u <-
  "http://pages.stern.nyu.edu/~adamodar/New_Home_Page/datafile/fundgrEB.html"
df1 <-
  as.data.frame(readHTMLTable(u, colClasses = c(
    "factor",
    "integer",
    rep("Percent", 3),
    which = 2,
...
df1$Industry.Name <-
  as.factor(str_replace(df1$Industry.Name, "[\\r*]", ""))
df1$Industry.Name <-
  as.factor(str_replace(df1$Industry.Name, "[\\t*]", ""))
df1$Industry.Name <-
  as.factor(str_replace(df1$Industry.Name, "[\\t*]", ""))
...
rownames(df2) <- df2$Industry.Name
...
df3 <-
  df3 %>% mutate(
    Expected.Growth.in.EBIT.log = log(Expected.Growth.in.EBIT + 5),
    Reinvestment.Rate.log = log(Reinvestment.Rate + 5)
  ) %>% na.omit()
df3 <- subset(df3, !(row.names(df3) %in% out3)) # For iterations
rownames(df3) <- df3$Industry.Name
fit3 <-
  lm(Expected.Growth.in.EBIT.log ~ Reinvestment.Rate.log, data = df3)
