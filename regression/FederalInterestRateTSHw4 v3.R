...
out1 <- c("1978-04-01","2009-01-01","2008-10-01","1958-01-01","1965-10-01","1975-01-01","1980-04-01")
...
df2 <-
  as.data.frame(df2 %>% dplyr::select(one_of(vars2)) %>% na.omit())
rownames(df2) <- df2$ObsDate
...
out2 <- mlreg(df = df2[vars2],
              fit = fit2,
              2)
fitco <- tsmlreg(df = df2[vars2],
        fit = fit2,
        num = 2, ts.stdres = ts.stdres2)
cat("*******************\n\nCochrane-Orcutt Manual procedure***************************\n")
cat("Rhohat =",rhohat <- Acf(ts.stdres2, plot = FALSE)$acf[2]) #or fitco$rho
n <- length(fit2$residuals)
#y[2:n] - rho * y[1:(n-1)]
df2$GDPpchstar <- df2$GDPpch - rhohat * (c(NA, df2$GDPpch[1:(n - 1)]))
df2$DomInvestpchstar <- df2$DomInvestpch - rhohat * (c(NA, df2$DomInvestpch[1:(n - 1)]))
df2$CPIpchstar <- df2$CPIpch - rhohat * (c(NA, df2$CPIpch[1:(n - 1)]))
df2$CorpProfitpchstar <- df2$CorpProfitpch - rhohat * (c(NA, df2$CorpProfitpch[1:(n - 1)]))
df21 <- df2
fit21 <- lm(GDPpchstar ~ DomInvestpchstar + CPIpchstar + CorpProfitpchstar, data = df21, na.action = na.omit)
vars21 <- colnames(model.frame(fit21, df21))
df21 <-
  as.data.frame(df2 %>% dplyr::select(one_of(vars21)) %>% na.omit())
ts.stdres21 <- ts(data=rstandard(fit21))
tsmlreg(df = df21[vars21],
        fit = fit21,
        num = 2, ts.stdres = ts.stdres21)
out2 <- mlreg(df = df21[vars21],
              fit = fit21,
              2)
powerT(df = df21[vars21],
       fit = fit21,
       2)
