...
long_df <-
  df1 %>% select(Qual_Score, DivSch1, BPA:UDW, BPA_Score:UDW_Score) %>% gather(BPA:UDW,
                                                                               key = "Group",
                                                                               value = "Valid",
                                                                               na.rm = TRUE) %>% gather(BPA_Score:UDW_Score,
                                                                                                        key = "Group1",
                                                                                                        value = "GrpScore",
                                                                                                        na.rm = TRUE)  

df1 <- long_df %>% select(Qual_Score,DivSch1,GrpScore) 
#df1$Group <- as.factor(df1$Group)
df1 <- df1 %>% select_if(function(x)
  ! all(is.infinite(x))) 
out1 <- c("374","1254","373","1252","1491","418")
df1 <- subset(df1,!(row.names(df1) %in% out1))
fit1 <-
  lm(Qual_Score ~  DivSch1 + GrpScore,
     data = df1)
...
df2 <-
  df2 %>%
  mutate(GrpScore.sqrt = sqrt(GrpScore), Qual_Score.sqrt = sqrt(Qual_Score))
fit2 <-
