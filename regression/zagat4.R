...
#rownames(df1) <- rownames(df1)
fit1 <- lm(Cost ~ Decor + Food + Service, data = df1)
...
fit2 <- lm(Cost ~ Decor + Food + Service, data = df2)
out1 <- NULL
out2 <- NULL
out2 <- mlreg(df = df2,
              fit = fit2,
...
df3 <- df2
response <- "Cost"
fit3 <- lm(Cost ~ Decor + Service, data = df3)
